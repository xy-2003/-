window.onload = function () {
    ...
    for(let i = 1;i <= 5;i++){
        ...
        cur_li.setAttribute('class','lbli')

        cur_img.src = '../比赛作品/img/图：' + i + '.jpg';
        ...
        bottom_div.name = i;
        bottom_div.setAttribute('class','lbdiv');
    ...
    szdiv[0].style.background = '#C10D0C';
    ...
    function sync_szdiv(){
        for(let i = 0;i < szdiv.length;i++){
            if(szdiv[i].name == sz[len - 1].id){
                szdiv[i].style.background = '#C10D0C';
            }
            else{
